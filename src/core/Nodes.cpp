...
static EvaluatedScene evaluateNodesInternal(const Project& project, int frame,
    ...
    for(std::size_t index=0;index<project.nodes.size();++index){
        ...
        else if(node.type=="arrow_new"){auto [iterator,inserted]=result.arrows.try_emplace(target);ArrowState& a=iterator->second;if(inserted)a.width=project.style.defaultArrowWidth;a.id=target;if(frame>=timing.startFrame)a.exists=true;}
        ...
        else if(node.type=="arrow_set_width"||node.type=="arrow_lerp_width"){ArrowState& a=result.arrows[target];a.id=target;add("arrow:"+target+":width",a.width,timing.startFrame,duration,p.value("value",project.style.defaultArrowWidth),easing);}
    }
