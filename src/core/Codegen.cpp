...
std::string compileLua(const Project& project) {
    ...
    for(const ScriptNode& node:project.nodes){if(!node.enabled)continue;const json p=json::parse(node.paramsJson);const std::string target=p.value("target","");std::string line;
        ...
        else if(node.type=="molecule_set_structure"){try{json snapshot=p.value("snapshot",json::object());if(snapshot.is_string())snapshot=json::parse(snapshot.get<std::string>());Point anchor{};if(const Molecule* molecule=project.molecule(target))anchor=molecule->coordinate().value_or(Point{});if(const auto value=decodedSnapshot(snapshot))line=target+":SetStructure("+moleculeTable(*value,anchor)+")";}catch(...){} }
        else if(node.type=="molecule_gradient_structure"){try{json start=p.value("start_snapshot",json::object()),end=p.value("end_snapshot",json::object());if(start.is_string())start=json::parse(start.get<std::string>());if(end.is_string())end=json::parse(end.get<std::string>());const auto startMolecule=decodedSnapshot(start),endMolecule=decodedSnapshot(end);if(startMolecule&&endMolecule){const Point anchor=startMolecule->coordinate().value_or(Point{});line=target+":LerpStructure("+moleculeTable(*startMolecule,anchor)+", "+moleculeTable(*endMolecule,anchor)+", "+std::to_string(p.value("frames",30))+", "+quote(p.value("easing","linear"))+")";}}catch(...){} }
        else if(node.type=="molecule_lerp_structure"){try{json atoms=p.value("atoms",std::string("{}"));if(atoms.is_string())atoms=json::parse(atoms.get<std::string>());Point anchor{};if(const Molecule* molecule=project.molecule(target))anchor=molecule->coordinate().value_or(Point{});std::ostringstream value;value<<target<<".LerpAtomsXY({";for(const auto& [id,atom]:atoms.items())value<<" ["<<quote(id)<<"]={"<<number(atom.value("x",0.0)-anchor.x)<<","<<number(atom.value("y",0.0)-anchor.y)<<"},";value<<" }, "<<p.value("frames",30)<<", "<<quote(p.value("easing","linear"))<<")";line=value.str();}catch(...){} }
