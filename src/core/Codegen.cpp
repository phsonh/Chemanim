...
std::string compileLua(const Project& project) {
    ...
    for(const ScriptNode& node:project.nodes){if(!node.enabled)continue;const json p=json::parse(node.paramsJson);const std::string target=p.value("target","");std::string line;
        if(node.type=="scene"){const Scene& s=project.scene;const Style& style=project.style;std::ostringstream value;value<<"chem.scene {\n    width = "<<s.width<<", height = "<<s.height<<",\n    logic_width = "<<s.logicWidth<<", logic_height = "<<s.logicHeight<<",\n    fps = "<<s.fps<<", view_zoom = "<<s.viewZoom<<",\n    font_pt = "<<style.fontPt<<", bond_length_pt = "<<style.bondLengthPt<<",\n    line_width_pt = "<<style.lineWidthPt<<", double_bond_spacing = "<<style.doubleBondSpacing<<",\n    electron_dot_radius_pt = "<<style.electronDotRadiusPt<<", default_arrow_width = "<<style.defaultArrowWidth<<",\n    background = "<<quote(s.background)<<", title = "<<quote(s.title)<<"\n}";line=value.str();}
        ...
        else if(node.type=="arrow_new"){if(!declaredArrows.contains(target)){line="local "+target+" = chem.NewArrow { thickness = "+number(project.style.defaultArrowWidth)+" }";declaredArrows.insert(target);}}
        ...
        else if(node.type=="arrow_set_width")line=target+".SetWidth("+number(p.value("value",1.5))+")";
        else if(node.type=="arrow_lerp_width")line=target+".LerpWidth("+number(p.value("value",1.5))+", "+std::to_string(p.value("frames",30))+", "+quote(p.value("easing","linear"))+")";
