...
public:
    ...
    py::dict depictAt(int frame, bool finalEffect) const {
        ...
        py::list bonds;for(const auto& bond:composite.bonds){py::dict item;item["id"]=bond.id;item["first"]=point(bond.first);item["second"]=point(bond.second);item["type"]=core::toString(bond.type);item["secondary_line_side"]=core::toString(bond.secondaryLineSide);item["line_spacing"]=bond.lineSpacing;item["converge_first"]=bond.convergeFirst;item["converge_second"]=bond.convergeSecond;item["convergence_length"]=bond.convergenceLength;bonds.append(item);}result["bonds"]=bonds;
