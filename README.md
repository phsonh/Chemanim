...
项目现在正在改成原生二维结构式：在编辑器里输入 SMILES 得到二维起稿，原子、键和坐标直接保存在 `.cmm` 中；手工拖动后的排版是权威数据，C++ 渲染器读取同一份数据画出最终画面，不会在运行时调用 RDKit。
...
- C++ 可以直接绘制单键、双键、三键、交替芳香键、实楔/虚楔和原子标签；
- 标签前会截断键线，默认样式以 Arial 10 pt 和 ACS Document 1996 为基准；
- 可以先输出 1920×1080 PNG 检查最终渲染。

旧的 PNG 模组和 MP4 管线暂时仍留在分支中作对照。原子位置、键长、键角、分支旋转、Pose、拓扑变化及其动画节点还没有迁移；这些会在二维绘制和编辑手感确认后再做。
...
在工具栏选择“从 SMILES 新建”，拖动原子后保存即可。仓库里有一个对乙酰氨基酚的 v2 示例：
