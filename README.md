...
- `.cmm` v6 保存稳定 atom/bond/adornment/node ID、单调不复用的 `creationSerial`、显式视觉键型和 ordered node sequence。
...
- 节点树是默认创作入口。只有选中有效的“创建/引用分子”节点时结构工具才可写；Set/Lerp 节点的画布拖动只修改节点目标，拖帧、播放和最终效果预览均为只读。
...
格式见 [tools/CMM_FORMAT.md](tools/CMM_FORMAT.md)，结构编辑冻结边界见 [docs/structure-editor-baseline.md](docs/structure-editor-baseline.md)，实现状态见 [docs/native-2d-status.md](docs/native-2d-status.md)，第三方来源见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
