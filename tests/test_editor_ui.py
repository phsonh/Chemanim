...
def test_real_structure_clipboard_crosses_molecules_and_structure_node_kinds():
    value=window();source=value.session.import_smiles("source","C1CCCC1")
    source_node=next(item["id"] for item in value.session.project()["nodes"]
                     if item["type"]=="molecule_set_structure" and item["params"]["target"]==source)
    value.refresh_all(source_node);value._node_selected(source_node);value.canvas.setFocus()
    QTest.keyClick(value.canvas,Qt.Key.Key_A,Qt.KeyboardModifier.ControlModifier)
    QTest.keyClick(value.canvas,Qt.Key.Key_C,Qt.KeyboardModifier.ControlModifier);QApplication.processEvents()
    assert QApplication.clipboard().mimeData().hasFormat(value.canvas.STRUCTURE_MIME)

    destination=value.session.import_smiles("destination","N")
    gradient=value.session.add_node("molecule_gradient_structure",json.dumps({"target":destination,"frames":30,"easing":"linear"}))
    value.refresh_all(gradient);value._node_selected(gradient);value.canvas.setFocus()
    before=active_structure(value);before_counts=(len(before["atoms"]),len(before["bonds"]))
    QTest.keyClick(value.canvas,Qt.Key.Key_V,Qt.KeyboardModifier.ControlModifier);QApplication.processEvents()
    after=active_structure(value)
    assert (len(after["atoms"]),len(after["bonds"]))==(before_counts[0]+5,before_counts[1]+5)
    assert len(value.canvas.selected_atoms)==5 and value.dirty
    value.undo();QApplication.processEvents();undone=active_structure(value)
    assert (len(undone["atoms"]),len(undone["bonds"]))==before_counts
    value.close()


def test_real_gradient_canvas_displays_external_reference_and_snaps_without_joining():
    value=window();active=value.session.import_smiles("active","CC");reference=value.session.import_smiles("reference","CC")
    value.session.add_node("molecule_set_position",json.dumps({"target":active,"x":-90.0,"y":0.0}))
    value.session.add_node("molecule_set_position",json.dumps({"target":reference,"x":90.0,"y":0.0}))
    gradient=value.session.add_node("molecule_gradient_structure",json.dumps({"target":active,"frames":30,"easing":"linear"}))
    value.refresh_all(gradient);value._node_selected(gradient);value._set_tool("move");value.canvas._sync_core_viewport()
    depiction=value.session.depict(False)
    assert len(depiction["reference_atoms"])==2 and f"data-molecule='{reference}'" in depiction["svg"]
    moving=depiction["atoms"][0]["center"];target=depiction["reference_atoms"][0]["center"]
    start=QPoint(round(moving["x"]),round(moving["y"]));end=QPoint(round(target["x"]),round(target["y"]))
    QTest.mousePress(value.canvas,Qt.MouseButton.LeftButton,pos=start)
    QTest.mouseMove(value.canvas,end,80);QApplication.processEvents()
    assert value.canvas._preview.get("text")=="原子重合 · 0×键长"
    QTest.mouseRelease(value.canvas,Qt.MouseButton.LeftButton,pos=end);QApplication.processEvents()
    snapshot=active_structure(value)
    assert len([item for item in snapshot["bonds"] if item.get("alive",True)])==1
    assert len(value.session.evaluated_project(value.session.end_frame)["molecules"])>=2
    value.close()


def test_real_position_node_drag_snaps_to_other_molecule_atom_and_parallel_bond():
    value=window();moving=value.session.import_smiles("moving","CC");reference=value.session.import_smiles("reference","CC")
    position=value.session.add_node("molecule_set_position",json.dumps({"target":moving,"x":-100.0,"y":0.0}))
    value.session.add_node("molecule_set_position",json.dumps({"target":reference,"x":100.0,"y":0.0}))
    value.refresh_all(position);value._node_selected(position);value._set_tool("move");value.canvas._sync_core_viewport()

    active_drawing=value.session.depict(False);source=active_drawing["atoms"][0]["center"]
    reference_state=next(item for item in value.session.evaluated_project(value.session.preview_frame)["molecules"] if item["id"]==reference)
    target_atom=reference_state["atoms"][0]
    target_screen=value.canvas.world_to_screen(QPointF(target_atom["x"],target_atom["y"]))
    start=QPoint(round(source["x"]),round(source["y"]));end=QPoint(round(target_screen.x()),round(target_screen.y()))
    QTest.mousePress(value.canvas,Qt.MouseButton.LeftButton,pos=start);QTest.mouseMove(value.canvas,end,80);QApplication.processEvents()
    assert value.canvas._preview.get("text")=="对象原子重合"
    QTest.mouseRelease(value.canvas,Qt.MouseButton.LeftButton,pos=end);QApplication.processEvents()
    value.undo();QApplication.processEvents()

    active_drawing=value.session.depict(False);shown=active_drawing["bonds"][0]
    source_mid=QPoint(round((shown["first"]["x"]+shown["second"]["x"])*.5),
                      round((shown["first"]["y"]+shown["second"]["y"])*.5))
    reference_state=next(item for item in value.session.evaluated_project(value.session.preview_frame)["molecules"] if item["id"]==reference)
    by_id={atom["id"]:atom for atom in reference_state["atoms"]};bond=reference_state["bonds"][0]
    world_mid=QPointF((by_id[bond["a"]]["x"]+by_id[bond["b"]]["x"])*.5,
                      (by_id[bond["a"]]["y"]+by_id[bond["b"]]["y"])*.5)
    target_screen=value.canvas.world_to_screen(world_mid);target_mid=QPoint(round(target_screen.x()),round(target_screen.y()))
    QTest.mousePress(value.canvas,Qt.MouseButton.LeftButton,pos=source_mid);QTest.mouseMove(value.canvas,target_mid,80);QApplication.processEvents()
    assert value.canvas._preview.get("text")=="对象键重合"
    QTest.mouseRelease(value.canvas,Qt.MouseButton.LeftButton,pos=target_mid);QApplication.processEvents()
    moved=next(item for item in value.session.evaluated_project(value.session.preview_frame)["molecules"] if item["id"]==moving)
    assert sorted((round(atom["x"],6),round(atom["y"],6)) for atom in moved["atoms"])==sorted((round(atom["x"],6),round(atom["y"],6)) for atom in reference_state["atoms"])
    value.close()
